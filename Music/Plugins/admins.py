...
@app.on_message(command(["pause", "وقف"]) & filters.group)
async def pause_cmd(_, message): 
    ...
    if not await is_active_chat(chat_id):
        return await message.reply_text("لايوجد شئ يعمل في الدردشة الصوتية")
...
@app.on_message(command(["resume", "استمرار", "كمل"]) & filters.group)
async def stop_cmd(_, message): 
    if message.sender_chat:
        return await message.reply_text("Kamu adalah __Admin Anonim__!\nKembalikan ke Akun Pengguna.") 
    permission = "can_manage_voice_chats"
    ...
    chat_id = message.chat.id
    if not await is_active_chat(chat_id):
        return await message.reply_text("**لا يوجد شئ يعمل**")
...
@app.on_message(command(["/end", "ايقاف", "اسكت", f"end@{BOT_USERNAME}", "e"]))
async def stop_cmd(_, message): 
    if message.sender_chat:
        return await message.reply_text("Kamu adalah __Admin Anonim__!\nKembalikan ke Akun Pengguna.") 
    permission = "can_manage_voice_chats"
    ...
    if await is_active_chat(chat_id):
        ...
        await remove_active_chat(chat_id)
        await music.pytgcalls.leave_group_call(chat_id)
        await message.reply_text(f"**🌹 تم ايقاف الاغنيه يا  {checking}!**") 
    else:
        return await message.reply_text("لا يوجد شئ يعمل")
    
@app.on_message(command(["skip", "تخطي", "اقلب"]) & filters.group)
async def stop_cmd(_, message): 
    if message.sender_chat:
        return await message.reply_text("Kamu adalah __Admin Anonim__!\nKembalikan ke Akun Pengguna.") 
    permission = "can_manage_voice_chats"
    ...
    if not await is_active_chat(chat_id):
        await message.reply_text("لا يوجد شئ يعمل ")
    else:
        task_done(chat_id)
        if is_empty(chat_id):
            await remove_active_chat(chat_id)
...
<b>🖋 اسم الاغنيه:</b> {title}
<b>⏰ الوقت:</b> {duration}
<b>🧸 اللي طلبها</b> {username}
""",
                )
                return

@app.on_message(command(["reload", "تحديث", "R"]) & filters.group)
async def reload(_, message):
    chat_id = message.chat.id
    await _.send_message(
    chat_id,
    "**🌹تم تحديث بوت الميوزك🌹**\n\n**🌹 تم تحديث الادمنيه 🌹**"
)
