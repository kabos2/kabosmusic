...
@app.on_message(command(["vplay", "كابوس فيديو", "تشغيل فيديو"]) & filters.group)
...
""",
    ...
    try:
        ...
        if b.status == "kicked":
            await message.reply_text(
                f"@{ubot.username} **تم حظر المساعد في هذه الدردشه** {message.chat.title}\n\n» **قم بفك الحظر وقوم بي الاستمتاع.**"
            )
            return
    ...
    if replied:
        if replied.video or replied.document:
            ...
            loser = await replied.reply("📥 **يتم تحميل الفيديو...**")
