    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await CallbackQuery.message.reply(
                f"🎻 تم ايقاف الاغنيه موقتآ {rpk}!", reply_markup=play_keyboard
            )
            await CallbackQuery.message.delete()
        else:
            await CallbackQuery.answer(f"لا يوجد شئ يعمل!", show_alert=True)
            return
    else:
        await CallbackQuery.answer(f"لا يوجد شئ يعمل!", show_alert=True)
...
""",
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            await CallbackQuery.answer(
                "لايوجد شئ متوقف مؤقتًا في الدردشة الصوتية",
                show_alert=True,
            )
            return
        else:
...
""",
        ...
        [
           InlineKeyboardButton("🇪🇬programmer", callback_data="sthjbt"),
        ],
        [
    ...
    await query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [        
                    InlineKeyboardButton(
                        "⁨كـ"ـابــوس الـ☻ـحـོـاڪـ"ـم🇻🇮⍣⃝˼⁩", url=f"https://t.me/QQQSTT"
                    ),
                ],
                [                  
                    InlineKeyboardButton(
