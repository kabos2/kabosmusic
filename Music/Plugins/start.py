...
def start_pannel():
    buttons = [
        [
            ...
            InlineKeyboardButton(text="قناة السورس🥀", url=f"https://t.me/{CHANNEL}"),
        ],
        [
            InlineKeyboardButton("الاوامر🥀", url="https://t.me/WD_RG/5"),
        ],
        [
            InlineKeyboardButton("⁨★ 𝙚𝙩𝙖𝙡𝙚𝙚 𝙢𝙪𝙨𝙞𝙘 ˹🥀˼⁩", url="https://t.me/ETALEE0"),
        ],
    ]
    return (
        "🥀 **{BOT_NAME} اهلا انا بوت تشغيل موسيقي في المحدثات الصواتية للاستماع بي موسيقي🎻**",
        buttons,
    )
    ...
    for member in message.new_chat_members:
        try:
            ...
            if member.id in SUDOERS:
                return await message.reply_text(
                    f"نــورت يـاغـالـي.💕🥀. [{member.mention}] جــروبـنـا المـتـواضـع + ممـنـوع الألفاظ الخارجه 😒 + مـمنـوع الحك يعم الحج 🌚 + مـمنـوع نشر اللينكات 😒⛔ • غـيـر كـده انـت شرفتـنا ياصحبي ـ💗ˣ᙮"
                )
            if member.id == ASSID:
            ...
            if member.id == BOT_ID:
                ...
                await message.reply_text(
                    f"""
🕺 ↯ ** هـالـي هـالـو عزيـزي المستخدم سأبـذل قصـاري جهـدي لأسمـعكـم موسـيقي عالـية الجـودة**

👮‍♂️↯ **لا تـنـسي ان تـقـوم ب تـعـيـنـي كـا مـشـرف **

🔥↯ **لا تـنـسي أعـطـاء البـوت صلاحـيه حـذف الرسـايـل **

🎻 ↯ **ارسـل تـشغيـل ليـتم انضـمام الحسـاب المـساعـد **

🎖 ↯ **ارسـل /help ليـتم عـرض أوامـر التـشـغيـل **
""",
...
**🧸 انا بـوت بـشـغـل اغـانـي فـي الدردشة الصوتية ويمكنه تشغيل مقاطع الفيديو في الدردشة الصوتية!**
...
async def okaybhai(_, CallbackQuery):
    ...
    await CallbackQuery.edit_message_text(
        text=f"شكرا لإضافتي {CallbackQuery.message.chat.title}.\n{BOT_NAME} متصل بالفعل.\n\nإذا كنت بحاجة إلى مساعدة أو كانت هناك مشكلة في البوت ، فيرجى الانضمام إلى مجموعتنا أو قناتنا.",
        reply_markup=InlineKeyboardMarkup(out[1]),
    )
