...
@Client.on_message(command(["/play", "كابوس" ,"شغل" ,"تشغيل", f"/play@{BOT_USERNAME}", "تشغيل"]))
async def play(_, message: Message):
    chat_id = message.chat.id
    if message.sender_chat:
        return await message.reply_text(
            """
اسف الا استطيع تنفيذ الامر انت لست ادمن اطلب من مطور البوت الصلاحيه او من مالك المجموعه.
"""
        )
...
"""
    ...
    if not a.can_delete_messages:
        await message.reply_text(
            "اسف ليس لدي الصلاحيات كامله عشان يشتغل"
            + "\n💗 حذف الرسايل"
        )
        return
    ...
    try:
        ...
        if b.status == "kicked":
            await message.reply_text(
                f"""
{ASSNAME}(@{ASSUSERNAME}) لقد قمت بحظر المساعد**{chat_title}**

الرجاء الغاء حظر المساعد لعمل البوت بطريقه صحيح
"""
            )
            return
        ...
        if message.chat.username:
            try:
                ...
                await message.reply(
                    f"{ASSNAME} تم أنضمام الحساب المسأعد بنجآح ي زوماله 🌹 ،،",
                )
...
"""
            ...
            try:
                ...
                await message.reply(f"{ASSNAME} تم أنضمام المساعد بنجآح ي زوماله . ✅",)
                await remove_active_chat(chat_id)
...
**لايمكن تخطي المده المسموح بها : {DURATION_LIMIT}**
**المده المسموح بها:** {duration}
"""
            )
        file_name = (
...
"""
        ...
        if len(message.command) < 2:
            ...
            hmo = await message.reply_text(
                """
<b>**يجب ان يكون بجانب الأمر أسم أو لينك الاغنيه**

**♡🥀اوامر التشغيل🥀♡**

`تشغيل او شغل او كابوس + أسم الاغنيه 🌹`
""",
            )
            return
        what = "Query Given"
...
"""
    ...
    if await is_active_chat(chat_id):
        ...
        m = await CallbackQuery.message.reply_photo(
            photo=thumb,
            caption=f"""
<b>🌹 تم اضافة الاغنيه في قائمة الانتظار</b>
