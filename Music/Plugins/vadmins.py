...
bttn = InlineKeyboardMarkup(
    [[InlineKeyboardButton("↩ - عوده - ↪", callback_data="cbmenu")]]
)
...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply(" **تم إيقاف الفيديو 🥀**")
...
@app.on_message(command(["vpause", "استنا", "انتظار"]) & filters.group)
@authorized_users_only
...
async def cbpause(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        ...
        await query.answer("❌ لا شيء يعمل الأن", show_alert=True)
