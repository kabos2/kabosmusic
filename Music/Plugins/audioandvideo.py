...
inl = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="الرجاء انتظار التحميل🎸", callback_data=f'down')
                ]   
            ]
        )
...
upl = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="في الانتظار🎸", callback_data=f'down')
                ]   
            ]
        )
...
__[kABOS | Programmer](https://t.me/QQQSTT)__ """    
    link = (x["webpage_url"])
...
def p_mark(link, channel):
    buttons= [
            ...
            [ 
                InlineKeyboardButton(text="اضغط هنا للذهاب لقناة اليوتيوب - 🥀", url=f'{channel}')
            ],
        ]
    return buttons    
    
async def send_file(CallbackQuery, med, filename, videoid, user_id, link, channel):
    await CallbackQuery.edit_message_text("جاري التحميل\n\nالفيديو او الاغنية التي طلبتها انتظر 🌹 .", reply_markup = upl)
    try:
